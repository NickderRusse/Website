window.onload = function() {	
	...
	var strokes = [
		...
		[0, 0, width_, 0],
		[width_, 0, width_, height_],
		[ width_, height_, 0, height_],
		[0, height_, 0, 0],
	...
	var rays = [];
		for(let i = 0; i < strokes.length * 3; i++){	
			rays[i] = new light(mid[0], height_ - length_);
		}
	var offrays = [];
		for(let i = 0; i < 8; i++){	
			let start = [];
			for(let j = 0; j < strokes.length * 3; j++){
				start[j] = new light(10, height_ - 5);
			}
			offrays[i] = start;
		}
	...
	var render = function(){
		...
		for(let i=0; i < rays.length; i++){
			offrays[0][i].loc[0] = rays[0].loc[0] + 8; 
			offrays[0][i].loc[1] = rays[0].loc[1]; 
			
			offrays[1][i].loc[0] = rays[0].loc[0] - 8; 
			offrays[1][i].loc[1] = rays[0].loc[1]; 
			
			offrays[2][i].loc[0] = rays[0].loc[0]; 
			offrays[2][i].loc[1] = rays[0].loc[1] + 8; 
			
			offrays[3][i].loc[0] = rays[0].loc[0]; 
			offrays[3][i].loc[1] = rays[0].loc[1] - 8;
			
			offrays[4][i].loc[0] = rays[0].loc[0] + 8; 
			offrays[4][i].loc[1] = rays[0].loc[1] + 8;
			
			offrays[5][i].loc[0] = rays[0].loc[0] + 8; 
			offrays[5][i].loc[1] = rays[0].loc[1] - 8;
			
			offrays[6][i].loc[0] = rays[0].loc[0] - 8; 
			offrays[6][i].loc[1] = rays[0].loc[1] + 8;
			
			offrays[7][i].loc[0] = rays[0].loc[0] - 8; 
			offrays[7][i].loc[1] = rays[0].loc[1] - 8;
		}
		...
		if(move){		
			if((rays[0].loc[0]<50) && (up == true)){
				up=false;
			}
			if(rays[0].loc[0]>width_-50 && up == false){
				up=true;
			}		
			for(var i = 0; i < rays.length; i++){
				if(up){
					rays[i].loc[0] -= width_ / 700;
				}else{
					rays[i].loc[0] += width_ / 700;
				}	
			}	
